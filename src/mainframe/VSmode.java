...
//메인 화면 만들기위한 클래스
class VSmode extends JFrame implements KeyListener, Runnable, ActionListener{
	...
	public void draw(){
		...
		if(mainCh.get_Hero_X_Point()<=framew/2) {                                 //캐릭터가 화면 왼쪽벽 위치
			...
			buffg.drawImage(tk.getImage("img/stage"+stage_Num+".gif"),  buffw-framew, mainCh.get_Hero_Y_Point()-buffh/2, buffw, buffh, this);
		}
		else {																		//나머지 상황일때
